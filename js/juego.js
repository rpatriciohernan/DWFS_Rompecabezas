...
function chequearSiGano() {
  ...
  for (var i=0; i<grilla.length; i++) {
    //determino cual es el valor de la primera columna
    var primerValor
    switch (i){
      case 0:
        primerValor = 1;
    ...
    for (var j=0; j<grilla[i].length; j++){
      // verifico el numero de pieza, si no es correcto, asigno false y dejo de recorrer
      if( grilla[i][j] != (primerValor+j) ) {
        gano = false;
        break;
      };
    };
    // si ya se detecto que no gano, dejo de recorrer
    if ( !gano ) {
      break;
    };
...
function posicionValida(fila, columna) {
    return (fila < grilla.length) && (columna < grilla[fila].length);
}
